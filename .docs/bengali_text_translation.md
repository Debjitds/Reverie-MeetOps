...
`login.usernameFormat` is reused by both login and registration validation.
...
as a dynamically generated error message.

If `{errorMsg}` is already localized or originates from a backend/API, preserve it exactly and do not translate it through a static dictionary.

### Copy-to-clipboard / AI agenda
...
It should not be translated by the static dictionary.
...
Do NOT add a static translation such as:

`actionResult.message → ...`

because its content is not fixed.

Instead:

- keep the dynamic message intact
- if true Bengali AI responses are required later, make the AI assistant locale-aware and request Bengali output when `bn` is active
- do not send this dynamic message through the static translation dictionary

This preserves the AI Assistant's actual semantics.
...
depending on the project's existing interpolation mechanism.
...
The external/dynamic error message remains unchanged unless the source itself supports localization.

---

# 56. Strings Observed as Unchanged in Bengali Reference

For static UI screenshots, some values are intentionally left in English.

For the newly added toast inventory, unlike the screenshot reference, the toast strings in Sections 41–52 are translated here because no Bengali screenshots were supplied for those notifications.

These toast translations are therefore **model-generated Bengali translations**, not screenshot-derived translations.
...
20. AI Assistant UI labels translate correctly.
...
25. Static toast notifications translate correctly.
26. Toast messages with dynamic placeholders preserve those placeholders.
27. Dynamic names, rooms, dates, times, emails and user data remain dynamic.
28. Switching Bengali → English restores the English UI.
29. Switching English → Bengali does not invoke Gemini or another translation API for static UI strings.
30. Browser refresh preserves the selected language.
31. Navigation between pages preserves the selected language.
32. No existing MeetOps functionality is broken by the localization implementation.
...
Toast translations in Sections 41–52 were generated specifically because Bengali screenshot references were not supplied for those toast messages.
...
Added complete static toast-notification translation inventory.

The codebase contains 78 toast triggers covering:
...
All static portions are translated in Sections 41–52.
